...
export async function loadInputPatents(input: TournamentInput): Promise<string[]> {
  ...
  if (input.sourceType === 'super_sector' && input.sourceId) {
    ...
    const filtered = candidates
      .filter(c => c.super_sector === input.sourceId)
      .slice(0, input.topN || 1000);
...
async function executeCluster(
  ...
  // For Round 2, also substitute round1Results
  if (roundContext?.roundNumber === 2 && roundContext.previousResults) {
    const round1ResultsJson = JSON.stringify(
      Array.from(roundContext.previousResults.entries()).map(([pid, result]) => ({
        patent_id: pid,
        ...result,
      })),
      null,
...
Return valid JSON only, no markdown code blocks.`;
  ...
  // Call LLM - use higher token limit for tournament evaluations (10 patents × ~15 questions each)
  const llmResult = await callLlm(fullPrompt, template.llmModel, SYSTEM_MESSAGE_STRUCTURED, 16384);
...
Return valid JSON only, no markdown code blocks.`;
  ...
  // Call LLM - use higher token limit for final synthesis
  const llmResult = await callLlm(fullPrompt, template.llmModel, SYSTEM_MESSAGE_STRUCTURED, 8192);
