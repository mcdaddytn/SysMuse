/**
 * Generate Computing-OS-Security Infringement Summary
 * Reads scores from cache for the computing-os-security patent set
 * against CrowdStrike, Palo Alto, Microsoft, SentinelOne, Cisco,
 * Trend Micro, Symantec, Check Point, McAfee.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  ...
  'microsoft': {
    label: 'Microsoft',
    slugProducts: [
      { slug: 'microsoft', product: 'microsoft-defender-for-endpoint' },
      { slug: 'microsoft', product: 'microsoft-defender-xdr' },
    ],
  },
  'sentinelone': {
    label: 'SentinelOne',
    slugProducts: [
      ...
      { slug: 'sentinelone', product: 'singularity-complete' },
    ],
  },
  'cisco': {
    label: 'Cisco',
    slugProducts: [
      { slug: 'cisco-systems', product: 'cisco-xdr' },
      { slug: 'cisco-systems', product: 'cisco-secure-firewall' },
      { slug: 'cisco', product: 'cisco-xdr' },
    ],
  },
  'trend-micro': {
    label: 'Trend Micro',
    slugProducts: [
      { slug: 'trend-micro', product: 'trend-vision-one' },
    ],
  },
  'symantec': {
    label: 'Symantec',
    slugProducts: [
      { slug: 'symantec', product: 'symantec-endpoint-security' },
    ],
  },
  'checkpoint': {
    label: 'Check Point',
    slugProducts: [
      { slug: 'check-point-software', product: 'harmony-endpoint' },
  ...
  'mcafee': {
    label: 'McAfee',
    slugProducts: [
      { slug: 'mcafee', product: 'mvision-endpoint' },
      { slug: 'mcafee', product: 'mvision-endpoint-detection-and-response' },
    ],
  },
};

const COMPANY_ORDER = ['crowdstrike', 'palo-alto', 'microsoft', 'sentinelone', 'cisco', 'trend-micro', 'symantec', 'checkpoint', 'mcafee'];
...
async function main() {
  ...
  const lines: string[] = [];
  lines.push('# Computing-OS-Security — Infringement Heat Map');
  lines.push('');
  ...
  fs.writeFileSync(path.join(outputDir, 'computing-os-security-infringement-heatmap.md'), md);
  console.log(`Written: computing-os-security-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'computing-os-security-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: computing-os-security-patent-company-matrix.csv`);
