/**
 * Generate Network-Auth-Access Infringement Summary
 * Reads scores from cache for the network-auth-access patent set
 * against Palo Alto, Fortinet, CrowdStrike, Okta, Zscaler, Cisco ISE.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  'palo-alto': {
    ...
    slugProducts: [
      ...
      { slug: 'palo-alto-networks', product: 'cortex-xsiam' },
  ...
  'crowdstrike': {
    ...
    slugProducts: [
      ...
      { slug: 'crowdstrike', product: 'falcon-complete-next-gen-mdr' },
      { slug: 'crowdstrike', product: 'falcon-pro' },
      { slug: 'crowdstrike', product: 'falcon-overwatch' },
      { slug: 'crowdstrike', product: 'threat-graph' },
    ],
  },
  'okta': {
    label: 'Okta',
  ...
  'zscaler': {
    ...
    slugProducts: [
      ...
      { slug: 'zscaler', product: 'advanced-threat-protection' },
      { slug: 'zscaler', product: 'zscaler-deception' },
    ],
  },
  'cisco-ise': {
    label: 'Cisco ISE',
...
const COMPANY_ORDER = ['palo-alto', 'fortinet', 'crowdstrike', 'okta', 'zscaler', 'cisco-ise'];
...
async function main() {
  const csvContent = fs.readFileSync('output/vendor-exports/network-auth-access-2026-04-06/vendor-targets.csv', 'utf-8');
  ...
  const lines: string[] = [];
  lines.push('# Network-Auth-Access — Infringement Heat Map');
  lines.push('');
  ...
  const outputDir = path.resolve('./output/vendor-exports/network-auth-access-2026-04-16');
  fs.writeFileSync(path.join(outputDir, 'network-auth-access-infringement-heatmap.md'), md);
  console.log(`Written: network-auth-access-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'network-auth-access-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: network-auth-access-patent-company-matrix.csv`);
