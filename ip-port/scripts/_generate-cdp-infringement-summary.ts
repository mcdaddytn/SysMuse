/**
 * Generate Computing-Data-Protection Infringement Summary
 * Reads scores from cache for the computing-data-protection patent set
 * against Varonis, Palo Alto, Microsoft, Dell, IBM, McAfee, Fortinet.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  'varonis': {
    label: 'Varonis',
    slugProducts: [
      { slug: 'varonis', product: 'data-security-platform' },
    ],
  },
  'palo-alto': {
    label: 'Palo Alto',
  ...
  'microsoft': {
    label: 'Microsoft',
    slugProducts: [
      { slug: 'microsoft', product: 'microsoft-defender-for-cloud' },
      { slug: 'microsoft', product: 'microsoft-defender-for-cloud-apps' },
    ],
  },
  'dell': {
    label: 'Dell',
    slugProducts: [
      { slug: 'dell-emc', product: 'powerprotect-data-manager' },
      { slug: 'dell-emc', product: 'powerprotect-dd-series-appliances' },
  ...
  'ibm': {
    label: 'IBM',
    slugProducts: [
      { slug: 'ibm-security', product: 'qradar-siem' },
      { slug: 'ibm-security', product: 'qradar-soar' },
    ],
  },
  'mcafee': {
    label: 'McAfee',
    slugProducts: [
      { slug: 'mcafee', product: 'total-protection-for-dlp' },
      { slug: 'mcafee', product: 'mvision-cloud-casb' },
...
const COMPANY_ORDER = ['varonis', 'palo-alto', 'microsoft', 'dell', 'ibm', 'mcafee', 'fortinet'];
...
async function main() {
  ...
  const lines: string[] = [];
  lines.push('# Computing-Data-Protection — Infringement Heat Map');
  lines.push('');
  ...
  fs.writeFileSync(path.join(outputDir, 'computing-data-protection-infringement-heatmap.md'), md);
  console.log(`Written: computing-data-protection-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'computing-data-protection-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: computing-data-protection-patent-company-matrix.csv`);
