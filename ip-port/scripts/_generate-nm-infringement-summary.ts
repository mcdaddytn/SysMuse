/**
 * Generate Network-Management Infringement Summary
 * Reads scores from cache for the network-management patent set
 * against Cisco, Arista, Juniper, SolarWinds, Extreme, ThousandEyes,
 * Fortinet, HPE Aruba.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  'cisco': {
    label: 'Cisco',
    slugProducts: [
      { slug: 'cisco-systems', product: 'cisco-catalyst-center' },
      { slug: 'cisco-systems', product: 'cisco-application-policy-infrastructure-controller-apic' },
      { slug: 'cisco', product: 'cisco-catalyst-center' },
      { slug: 'cisco', product: 'cisco-application-policy-infrastructure-controller-apic' },
    ],
  },
  'arista': {
    label: 'Arista',
  ...
  'juniper': {
    ...
    slugProducts: [
      ...
      { slug: 'juniper', product: 'apstra-data-center-director' },
      { slug: 'juniper', product: 'mist-ai' },
    ],
  },
  'solarwinds': {
    label: 'SolarWinds',
    slugProducts: [
      { slug: 'solarwinds', product: 'network-performance-monitor-with-aiops' },
      { slug: 'solarwinds', product: 'network-performance-monitor' },
    ],
  },
  'extreme': {
    label: 'Extreme',
    slugProducts: [
      { slug: 'extreme-networks', product: 'extremecloud-iq' },
    ],
  },
  'thousandeyes': {
    label: 'ThousandEyes',
    slugProducts: [
      { slug: 'thousandeyes-cisco', product: 'thousandeyes-platform' },
    ],
  },
  'fortinet': {
    label: 'Fortinet',
  ...
  'hpe-aruba': {
    label: 'HPE Aruba',
    slugProducts: [
      { slug: 'hpe-hewlett-packard-enterprise', product: 'hpe-aruba-networking-central' },
      { slug: 'hpe', product: 'hpe-aruba-networking-central' },
    ],
  },
};

const COMPANY_ORDER = ['cisco', 'arista', 'juniper', 'solarwinds', 'extreme', 'thousandeyes', 'fortinet', 'hpe-aruba'];
...
async function main() {
  const csvContent = fs.readFileSync('output/vendor-exports/network-management-2026-04-16/vendor-targets.csv', 'utf-8');
  ...
  const lines: string[] = [];
  lines.push('# Network-Management — Infringement Heat Map');
  lines.push('');
  ...
  const outputDir = path.resolve('./output/vendor-exports/network-management-2026-04-16');
  fs.writeFileSync(path.join(outputDir, 'network-management-infringement-heatmap.md'), md);
  console.log(`Written: network-management-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'network-management-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: network-management-patent-company-matrix.csv`);
