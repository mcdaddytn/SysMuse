/**
 * Generate Computing-Systems Infringement Summary
 * Reads scores from cache for the computing-systems patent set
 * against Intel, AMD, NVIDIA, Dell, HPE.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  'intel': {
    label: 'Intel',
    slugProducts: [
      { slug: 'intel', product: '5th-gen-intel-xeon-scalable-processors' },
      { slug: 'intel', product: 'intel-xeon-6-processor-family' },
      { slug: 'intel', product: 'intel-ethernet-800-series' },
    ],
  },
  'amd': {
    label: 'AMD',
    slugProducts: [
      { slug: 'amd', product: 'amd-epyc-9004-series' },
    ],
  },
  'nvidia': {
    label: 'NVIDIA',
    slugProducts: [
      { slug: 'nvidia', product: 'grace-hopper-superchip' },
      { slug: 'nvidia', product: 'h100-tensor-core-gpu' },
    ],
  },
  'dell': {
    label: 'Dell',
    slugProducts: [
      { slug: 'dell-emc', product: 'poweredge-r7525-server-technical-guide' },
      { slug: 'dell', product: 'poweredge-r7525-server-technical-guide' },
    ],
  },
  'hpe': {
    label: 'HPE',
    slugProducts: [
      { slug: 'hpe', product: 'hpe-proliant-dl380-gen11-quickspecs' },
      { slug: 'hpe-hewlett-packard-enterprise', product: 'hpe-proliant-dl380-gen11-quickspecs' },
      { slug: 'hewlett-packard-enterprise', product: 'hpe-proliant-dl380-gen11-quickspecs' },
    ],
  },
};

const COMPANY_ORDER = ['intel', 'amd', 'nvidia', 'dell', 'hpe'];
...
async function main() {
  const csvContent = fs.readFileSync('output/vendor-exports/computing-systems-2026-04-16/vendor-targets.csv', 'utf-8');
  ...
  const lines: string[] = [];
  lines.push('# Computing-Systems — Infringement Heat Map');
  lines.push('');
  ...
  const outputDir = path.resolve('./output/vendor-exports/computing-systems-2026-04-16');
  fs.writeFileSync(path.join(outputDir, 'computing-systems-infringement-heatmap.md'), md);
  console.log(`Written: computing-systems-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'computing-systems-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: computing-systems-patent-company-matrix.csv`);
