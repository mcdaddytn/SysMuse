/**
 * Generate Network-Secure-Compute Infringement Summary
 * Reads scores from cache for the network-secure-compute patent set
 * against Zscaler, Palo Alto, Cisco, Fortinet, Cloudflare, Nutanix.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  'zscaler': {
    label: 'Zscaler',
    slugProducts: [
      { slug: 'zscaler', product: 'zscaler-zero-trust-exchange' },
      { slug: 'zscaler', product: 'zscaler-internet-access-zia' },
      { slug: 'zscaler', product: 'zscaler-cloud-firewall' },
    ],
  },
  'palo-alto': {
    label: 'Palo Alto',
    slugProducts: [
      { slug: 'palo-alto-networks', product: 'prisma-cloud' },
      { slug: 'palo-alto-networks', product: 'cortex-xdr' },
      { slug: 'palo-alto-networks', product: 'vm-series-virtual-next-generation-firewall' },
    ],
  },
  'cisco': {
    label: 'Cisco',
    slugProducts: [
      ...
      { slug: 'cisco-systems', product: 'cisco-tetration-analytics' },
      { slug: 'cisco', product: 'cisco-application-centric-infrastructure-aci' },
    ],
  },
  'fortinet': {
    label: 'Fortinet',
    slugProducts: [
      { slug: 'fortinet', product: 'fortigate-vm' },
      { slug: 'fortinet', product: 'fortigate-vm-series' },
  ...
  'cloudflare': {
    label: 'Cloudflare',
    slugProducts: [
      { slug: 'cloudflare', product: 'cloudflare-gateway' },
      { slug: 'cloudflare', product: 'magic-transit' },
    ],
  },
  'nutanix': {
    label: 'Nutanix',
    slugProducts: [
      { slug: 'nutanix', product: 'flow-network-security' },
      { slug: 'nutanix', product: 'nutanix-cloud-infrastructure-nci' },
      { slug: 'nutanix', product: 'nutanix-flow-network-security' },
    ],
  },
};

const COMPANY_ORDER = ['zscaler', 'palo-alto', 'cisco', 'fortinet', 'cloudflare', 'nutanix'];
...
async function main() {
  const csvContent = fs.readFileSync('output/vendor-exports/network-secure-compute-2026-04-16/vendor-targets.csv', 'utf-8');
  ...
  const lines: string[] = [];
  lines.push('# Network-Secure-Compute — Infringement Heat Map');
  lines.push('');
  ...
  const outputDir = path.resolve('./output/vendor-exports/network-secure-compute-2026-04-16');
  fs.writeFileSync(path.join(outputDir, 'network-secure-compute-infringement-heatmap.md'), md);
  console.log(`Written: network-secure-compute-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'network-secure-compute-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: network-secure-compute-patent-company-matrix.csv`);
