/**
 * Generate Network-Protocols Infringement Summary
 * Reads scores from cache for the network-protocols patent set
 * against Cisco, Juniper, Arista, Nokia, Huawei.
 */
...
const COMPANY_PRODUCTS: Record<string, { label: string; slugProducts: Array<{ slug: string; product: string }> }> = {
  'cisco': {
    label: 'Cisco',
    slugProducts: [
      { slug: 'cisco-systems', product: 'cisco-asr-1000-series-routers' },
      { slug: 'cisco-systems', product: 'cisco-catalyst-9000-series' },
      { slug: 'cisco-systems', product: 'cisco-crosswork-network-services-orchestrator-nso' },
      { slug: 'cisco-systems', product: 'cisco-catalyst-3850-series' },
      { slug: 'cisco-systems', product: 'cisco-catalyst-9300-series' },
      { slug: 'cisco', product: 'cisco-asr-1000-series-routers' },
    ],
  },
  'juniper': {
    label: 'Juniper',
    slugProducts: [
      { slug: 'juniper-networks', product: 'vmx-virtual-router' },
      { slug: 'juniper-networks', product: 'cloud-native-contrail-networking' },
  ...
  'arista': {
    ...
    slugProducts: [
      ...
      { slug: 'arista-networks', product: '7800r4-series' },
    ],
  },
  'nokia': {
    label: 'Nokia',
    slugProducts: [
      { slug: 'nokia', product: '7250-ixr' },
      { slug: 'nokia', product: 'virtualized-service-router-vsr' },
  ...
  'huawei': {
    label: 'Huawei',
    slugProducts: [
      { slug: 'huawei', product: 'imaster-nce-autonomous-network-management-and-control-system' },
      { slug: 'huawei', product: 'smartax-ma5600t' },
    ],
  },
};

const COMPANY_ORDER = ['cisco', 'juniper', 'arista', 'nokia', 'huawei'];
...
async function main() {
  const csvContent = fs.readFileSync('output/vendor-exports/network-protocols-2026-04-16/vendor-targets.csv', 'utf-8');
  ...
  const lines: string[] = [];
  lines.push('# Network-Protocols — Infringement Heat Map');
  lines.push('');
  ...
  const outputDir = path.resolve('./output/vendor-exports/network-protocols-2026-04-16');
  fs.writeFileSync(path.join(outputDir, 'network-protocols-infringement-heatmap.md'), md);
  console.log(`Written: network-protocols-infringement-heatmap.md (${md.length} chars)`);
  ...
  fs.writeFileSync(path.join(outputDir, 'network-protocols-patent-company-matrix.csv'), csvLines.join('\n'));
  console.log(`Written: network-protocols-patent-company-matrix.csv`);
